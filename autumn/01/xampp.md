...
まずは[こちらのサイト](https://www.apachefriends.org/jp/index.html)にアクセスしてWindows向け、Mac向けなどがあるので適当な方をクリックしてインストーラをダウンロードしてください。できれば上の方のダウンロードタブから、それぞれのOSにあったところのさらにダウンロードというところからバージョンの1.7.3をダウンロードすることをお勧めします。
...
ここまで完了したら、一旦こちらにアクセスしてください。http://localhost  
...
できたら、http://localhost/hello.phpにアクセスしてください。正しく表示できましたか？これで基本的なXAMPPの設定は完了です。それでは次に日本語の設定など細かい設定を説明していきます。
...
管理者を設定していきます。
XAMPPの設定画面にアクセスしてください。Macのかたはhttp://localhost/xampp/、Windowsの方はhttp://localhost/xampp/splash.phpになります。
...
* `su -` rootユーザになる
* `> Password:` パスワードを入れる
* `/Applications/XAMPP/xamppfiles/xampp security` をコール
...
> #### 参考　もし`su -`に失敗したら
> ルートユーザが有効になっていない可能性があります。こちらの[アップルのサポート](https://support.apple.com/ja-jp/HT204012)のルートユーザを有効にする方法の部分を順に行ってください。

#### Windowsの場合

http://localhost/xampp/splash.phpの左側のセキュリティタブのページにてhttp://localhost/security/xamppsecurity.phpのリンクをクリックしてください。
...
今回の開発ではphpmyadminと呼ばれるブラウザ上でデータベースを操作したりすることのできるツールを使用していきますphpMyAdminにアクセスしましょう。http://localhost/phpmyadmin/index.php
